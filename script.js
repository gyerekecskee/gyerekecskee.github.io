...
document.getElementById('party-form').addEventListener('submit', function(e) {
    ...
    //alert('Quiz submitted! Add your scoring logic here.');
    // Get selected options
    const selectedOptions = Array.from(document.querySelectorAll('input[name="q5"]:checked'))
    .map(option => option.value);
    console.log(selectedOptions);
...
// Show feedback
const feedback = document.getElementById('f8');
if (isCorrect) {
    feedback.textContent = 'Helyes válasz. Valóban a palacsinta volt a témája a bulinak, már amennyiben azt témának lehet nevezni hogy az volt az étel. Ennek a kérdésnek a betűje az L.';
    feedback.classList.add('correct');
    feedback.classList.remove('incorrect');
} else {
    feedback.textContent = 'Rossz válasz.';
